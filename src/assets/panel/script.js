...
async function initiatePanel(proxySettings) {
    ...
    await loadBackgroundOnInit();   // 加载背景配置
...
function populatePanel(proxySettings) {
    ...
    textareaElements.forEach(elm => {
        ...
        if (key === 'linkUrl') {
			...
			rowsCount = Math.min(lines * 1, 12); // 实际行数 * 1，最多 12 行
        } else {
		...
		if (limitFields.includes(key)) {
			rowsCount = Math.min(rowsCount, 20); //最多 20 行
		}
	...
	// 加载后仅对 linkUrl 自适应高度
	const linkUrlTextarea = document.getElementById('linkUrl');
	if (linkUrlTextarea) {
		linkUrlTextarea.style.height = 'auto';
		linkUrlTextarea.style.height = linkUrlTextarea.scrollHeight + 'px';
	}
}
...
function initiateForm() {
    ...
    // 当设置/订阅/背景主折叠关闭时，自动关闭所有子折叠
	document.querySelectorAll('.cf-usage-stats.cf-stat-inside-card.padding > details').forEach(detail => {
		detail.addEventListener('toggle', function() {
			if (!this.open) {
				this.querySelectorAll('details').forEach(sub => sub.open = false);
			}
		});
	});
	
	// 当 VLESS - Trojan 折叠打开时，重新调整 linkUrl 高度
	const vlessDetails = document.querySelector('details.details');
	if (vlessDetails) {
		vlessDetails.addEventListener('toggle', function() {
			if (this.open) {
				const linkUrlTextarea = document.getElementById('linkUrl');
				if (linkUrlTextarea) {
					linkUrlTextarea.style.height = 'auto';
...
async function chooseAndOpenQR(path, app, tag, title, singboxType) {
    ...
    qrcodeTitle.textContent = title + (useLink ? ' (Link IPs)' : ' (Clean IPs)');
...
function addUdpNoise(isManual, noiseIndex, udpNoise) {
    ...
    container.className = "inner-container-noise";
...
function generateUdpNoise(event) {
    ...
    const noisePacket = event.target.closest(".inner-container-noise").querySelector('[name="udpXrayNoisePacket"]');
...
function deleteUdpNoise(event) {
    ...
    if (!confirmReset) return;
    event.target.closest(".inner-container-noise").remove();
    globalThis.xrayNoiseCount--;
}
...
function applyBackgroundToPage(image, position, opacity) {
    // 只有非内部路由才立即渲染（避免重置场景多一次 /background-image 请求）
    if (image && image !== '/background-image') {
    ...
    // 1.5 秒后切换为 worker 路由版本（与刷新页面后保持一致）
    setTimeout(() => {
...
function loadBackgroundOnInit() {
    return fetch('/panel/background-config')
        .then(res => res.json())
        .then(data => {
...
function updateCloudflareUI(usageData, kvData) {
    ...
    if (usageData.success && usageData.body) {
        ...
        document.getElementById('cf-request-percent').textContent = (usageData.body.percentage || '0') + '%';
    } else {
    ...
    if (kvData.success && kvData.body) {
        document.getElementById('cf-kv-read-used').textContent = (kvData.body.readTotal || 0).toLocaleString();
        document.getElementById('cf-kv-read-percent').textContent = (kvData.body.readPercentage || '0') + '%';
        document.getElementById('cf-kv-write-used').textContent = (kvData.body.writeTotal || 0).toLocaleString();
        document.getElementById('cf-kv-write-percent').textContent = (kvData.body.writePercentage || '0') + '%';
        ...
        if (kvData.body.details && kvData.body.details.length) {
            detailsHtml = kvData.body.details.map(ns =>
                `<div style="padding: 5px 0;">
                    ...
                    读: ${ns.read || 0} | 写: ${ns.write || 0} | 删: ${ns.delete || 0} | 列: ${ns.list || 0}
                </div>`
            ).join('');
        } else {
            detailsHtml = '暂无数据';
        ...
        document.getElementById('cf-kv-details').textContent = '获取失败，请检查';
    }
}
...
// 加载 Cloudflare 配置到表单并保存到全局
function loadCloudflareConfig() {
    // 返回 fetch 的 Promise
    return fetch('/panel/cloudflare-config')
        .then(res => res.json())
        .then(data => {
            if (data.success && data.body) {
                ...
                document.getElementById('cfQueryUrl').value = data.body.queryUrl || '';
                globalThis.cfQueryUrl = data.body.queryUrl || '';
            }
        })
        .catch(err => console.error('加载 Cloudflare 配置失败:', err));
}
...
function renderCustomGroups() {
    ...
    if (groups.length === 0) {
        list.innerHTML = `<div style="color: var(--hr-text-color); justify-content: center; padding: 8px;">暂无自定义分组规则</div>`;
    } else {
        list.innerHTML = groups.map((g, i) => `
            <div class="custom-group-item" data-index="${i}">
                <input type="checkbox" class="group-checkbox" data-index="${i}">
                <span class="group-info"><strong>${g.name}</strong> — ${g.url}</span>
...
function moveSelected(direction) {
    ...
    const selectedItems = indices.map(i => groups[i]);
    ...
    if (direction === 'top') {
        ...
        // 上移：插入到原第一个选中项之前
        insertPos = firstIdx - 1;
        if (insertPos < 0) insertPos = 0;
    } else if (direction === 'down') {
        // 下移：插入到原最后一个选中项之后
        // 原 lastIdx 位置删除后，插入位置应为 lastIdx - selectedCount + 2
        insertPos = lastIdx - selectedCount + 2;
        if (insertPos > groups.length) insertPos = groups.length;
    }
    ...
    if (direction === 'bottom') {
        ...
        groups.splice(insertPos, 0, ...selectedItems);
    }
