...
export async function getXrCustomConfigs(isFragment: boolean, useLink: boolean = false): Promise<Response> {
	...
	if (customGroups.length > 0) {
		...
		// ===== 合并同名分组：按名称聚合规则 =====
		const mergedGroups = new Map<string, { rules: string[] }>();
		for (const group of customGroups) {
			if (!group.url) continue;
			const rules = await fetchCustomGroupRules(group.url);
			if (rules.length === 0) continue;
			if (!mergedGroups.has(group.name)) {
				mergedGroups.set(group.name, { rules: [] });
			}
			mergedGroups.get(group.name)!.rules.push(...rules);
		}

		// 清除已有自定义 balancer（避免重复添加）
		if (mainConfig.routing.balancers) {
			// 只保留不属于当前合并分组名称的 balancer（用于更新）
			const currentGroupNames = new Set(mergedGroups.keys());
			mainConfig.routing.balancers = mainConfig.routing.balancers.filter(b => !currentGroupNames.has(b.tag));
		}
		...
		// 处理每个合并后的分组
		for (const [name, { rules }] of mergedGroups) {
			// 1. 规则去重
			const uniqueRules = [...new Set(rules)];

			// 2. 创建 balancer（一个名称一个）
			...
			for (const rule of uniqueRules) {
				...
				let xrayRule: RoutingRule = { type: "field", outboundTag: name };
				switch (ruleType) {
					case 'DOMAIN':
