...
export function generateRemark(
    ...
    // 确定分组键：若有备注则用备注，否则用 'Clean'
    const groupKey = remark || 'Clean';
    // 分组内计数
    const count = (remarkCounter.get(groupKey) || 0) + 1;
    remarkCounter.set(groupKey, count);

    let prefixSymbol = '☁ ';
    let displayName = 'Clean';

    if (remark && /^[A-Z]{2}$/.test(remark)) {
