...
import { getConfigAddresses, generateRemark, getProtocols, fetchCustomGroupRules, isSupportedClashRule, resetRemarkCounter } from '@utils';
	...
	for (const [name, { rules }] of mergedGroups) {
		...
		for (const rule of rules) {
			...
			if (routeTypes.includes(ruleType) && parts.length < 3) {
				finalRule = `${trimmed},${name}`; // 使用分组名作为目标组
			}
